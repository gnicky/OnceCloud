...
public class IPUtils {
	...
	public final static boolean invalidIP(String ipaddr) {
		return ObjectUtils.invalid(ipaddr) ? true : !IP_PATTERN.matcher(ipaddr).matches();
	}
	...
	public final static Inet4Address getInet4Address(String hostname) {
		try {
			Inet4Address ipaddr = (Inet4Address) Inet4Address
					.getByName(hostname);
			return invalidIP(ipaddr.getHostAddress()) ? UNKNOWN_HOST : ipaddr;
		} catch (UnknownHostException e) {
			m_logger.error(Constants.UTILS_IP + e.getMessage());
			return UNKNOWN_HOST;
		}
	}
