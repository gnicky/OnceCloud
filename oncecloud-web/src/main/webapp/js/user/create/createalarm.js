...
$('#createAlarmAction').on('click', function (event) {
    ...
    if (flag) {
        ...
        $.ajax({
            type: 'get',
            url: '/AlarmAction',
            data: 'action=create&alarmUuid=' + alarmUuid + '&alarmName=' + alarmName + '&alarmType=' + alarmType + '&alarmIsalarm='
                + alarmIsalarm + '&alarmTouch=' + alarmTouch + '&alarmPeriod=' + alarmPeriod + '&alarmRules=' + alarmRules + '&alarmThreshold=' + alarmThreshold,
            dataType: 'text',
                        ...
                        buttons: {
                            main: {
                                ...
                                callback: function () {
                                    ...
                                    $("#tablebody").prepend('<tr rowid="' + alarmUuid + '"><td class="rcheck"><input type="checkbox" name="alrow"></td><td name="console">' + showstr + '</td><td name="alarmName">'
                                        + alarmName + '</td>' + stateStr + '<td name="alarmType">'
                                        + typeStr + '</td><td name="alarmPeriod">'
                                        + alarmPeriod + '分钟</td><td name="alarmModify">是</td>' + '<td name="createtime" class="time">1分钟以内</td></tr>');
                                }
                            }
                        }
                    });
...
$("#policy").change(function () {
    ...
    if ($("#policy").find("option:selected").val() == "0") {
        ...
        $("#meter").append("<option value='40'>后端 HTTP 1xx 响应数</option>");
        $("#meter").append("<option value='42'>后端 HTTP 2xx 响应数</option>");
        $("#meter").append("<option value='44'>后端 HTTP 3xx 响应数</option>");
        $("#meter").append("<option value='46'>后端 HTTP 4xx 响应数</option>");
        $("#meter").append("<option value='48'>后端 HTTP 5xx 响应数</option>");
        ...
        $("#meter").append("<option value='60'>后端 HTTP 1xx 响应数</option>");
        $("#meter").append("<option value='62'>后端 HTTP 2xx 响应数</option>");
        $("#meter").append("<option value='64'>后端 HTTP 3xx 响应数</option>");
        $("#meter").append("<option value='66'>后端 HTTP 4xx 响应数</option>");
        $("#meter").append("<option value='68'>后端 HTTP 5xx 响应数</option>");
