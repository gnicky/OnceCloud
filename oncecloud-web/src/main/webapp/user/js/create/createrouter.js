$(document).ready(function () {
    ...
    function loadFWList() {
        $.ajax({
            ...
            async: false,
            url: '/RouterAction',
            data: 'action=getablefws',
            dataType: 'json',
            success: function (response) {
                var deitem = response.first;
                $('#rt_firewall').html('<option value="' + deitem.uuid + '">' + decodeURI(deitem.rtname) + '</option>');
                var detype = $('input:radio').eq(0).val();
                setTypeInfo(detype);
                var array = new Array();
                array = response.list;
                for (var i = 0; i < array.length; i++) {
                    $('#rt_firewall').append('<option value="' + array[i].uuid + '">fw-' + array[i].uuid.substring(0, 8) + '&nbsp;(' + array[i].rtname + ')</option>');
                }
                ;
    ...
    function createRouter(rtuuid, name, capacity, firewall) {
        $.ajax({
            type: 'post',
            url: '/RouterAction',
            data: 'action=create&uuid=' + rtuuid + '&name=' + name + '&capacity=' + capacity + '&fwuuid=' + firewall,
            dataType: 'json',
            success: function (array) {
            },
            error: function () {
