$(document).ready(function () {
    ...
    $("#applyeipAction").on('click', function (event) {
        ...
        if (valid) {
            ...
            $.ajax({
                type: 'get',
                url: '/EipAction',
                data: 'action=quota&count=' + eipCount + '&size=' + eipTotalSize,
                dataType: 'text',
                cache: false,
    ...
    $("#apply-form").validate({
        ...
        messages: {
            eip_name: {
                required: "<span class='unit'>公网IP名不能为空</span>",
    ...
    function applyEip(eipName, eipSize) {
        ...
        $.ajax({
            type: 'post',
            url: '/EipAction',
            data: 'action=apply&eipName=' + eipName + '&eipSize=' + eipSize + '&eipUuid=' + eipUuid,
            dataType: 'json',
