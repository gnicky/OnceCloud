$(document).ready(function () {
    ...
    $("#createfirewallAction").on('click', function (event) {
        ...
        if (valid) {
            var firewallName = document.getElementById("firewall_name").value;
            $.ajax({
                type: 'get',
                url: '/FirewallAction',
                data: 'action=quota&count=1',
                dataType: 'text',
    ...
    function createFirewall(firewallName) {
        ...
        $.ajax({
            type: 'post',
            url: '/FirewallAction',
            data: 'action=create&firewallName=' + firewallName + '&firewallUuid=' + firewallUuid,
            dataType: 'json',
