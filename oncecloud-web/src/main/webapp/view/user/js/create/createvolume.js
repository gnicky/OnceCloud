$(document).ready(function () {
    ...
    $("#size").on('focusout', function (event) {
        ...
        var vsize = 10;
        if (/^[1-5]{0,1}[1-9]{1}[0]{1}$/.test(document.getElementById("size").value)) {
    ...
    $("#createvolumeAction").on('click', function (event) {
        ...
        if (valid) {
            var volumeName = document.getElementById("volume_name").value;
            var volumeCount = parseInt(document.getElementById("count").value, 10);
            var volumeSize = parseInt(document.getElementById("size").value, 10);
            var volumeTotalSize = volumeSize * volumeCount;
            $.ajax({
                type: 'post',
                url: '/VolumeAction',
                data: 'action=quota&count=' + volumeCount + '&size=' + volumeTotalSize,
                dataType: 'text',
    ...
    function createVolume(volumeuuid, volumeName, volumeSize) {
        ...
        $.ajax({
            type: 'post',
            url: '/VolumeAction',
            data: "action=create&volumeuuid=" + volumeuuid + "&volumeName=" + volumeName + "&volumeSize=" + volumeSize,
            dataType: 'json',
