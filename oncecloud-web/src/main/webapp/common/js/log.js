$(document).ready(function() {
	...
	function getLogList(logStatus, append, start) {
		$.ajax({
			type: 'get',
			url: '/LogAction',
			...
			success: function(jsonArray) {
				...
				if (jsonArray.length == 0) {
					...
					var iconStr = new Array("cloud", "inbox", "camera", "globe", "globe", 
						"flash", "record", "barcode", "fullscreen", "random", "camera", "flash", "user", "tint", "hdd", "road", "tasks", "fullscreen", "globe", "indent-left", "thumbs-up", "flag", "inbox");
					...
					for (var i = 0; i < jsonArray.length; i++) {
						...
						if (logStatus < 2) {
							...
							logElapseSpan = '<span class="log-elapse"><span class="glyphicon glyphicon-'
								+ statusIconStr[logStatus] + '"></span>' + logElapseStr + '</span></div>';
						}
						var logIcon = '<span class="glyphicon glyphicon-' + iconStr[logObject] +'"></span>';
						var alertItem = '<li class="log-item"><div class="alert alert-' + statusStr[logStatus] + ' alert-item">' + logIcon 
							+ '<span class="log-name">' + logActionStr + logObjectStr + '</span>'
							+ '<span class="log-time"><span class="glyphicon glyphicon-time"></span>' + logTime + '</span>'
							+ logElapseSpan;
						...
						for (var j = 0; j < logInfo.length; j++) {
							...
							if (logInfoObj.value == "") {
								...
								logValueStr = '<dd><a>' + decodeURI(logInfoObj.value) + '</a></dd>';
							}
							var logInfoStr = '<dt class="log-dt">' + decodeURI(logInfoObj.key) + '&nbsp;:</dt>'
								+ logValueStr;
