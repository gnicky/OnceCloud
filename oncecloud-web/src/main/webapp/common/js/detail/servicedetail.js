$(document).ready(function() {
	...
	function getQuestionDetail() {
		...
		$.ajax({
			type: 'get',
			url: '/QAAction',
			data: 'action=questiondetail&qaid='+qaId,
			dataType: 'json',
	...
	$('#platformcontent').on('click', '#area-submit', function(event) {
		event.preventDefault();
		var content = document.getElementById("qa-reply").value;
		var qaId = $("#platformcontent").attr("qaId");
		var userName = $("#platformcontent").attr("userName");
		$.ajax({
			type: 'post',
			url: '/QAAction',
			data: 'action=addreply&qaid='+qaId+'&content='+content,
			dataType: 'json',
			success: function(array) {
				if (array.length == 1) {
					var obj = array[0];
	...
	function getAnswerList() {
		...
		$.ajax({
			type: 'get',
			url: '/QAAction',
			data: 'action=getanswer&qaid='+qaId,
			dataType: 'json',
			success: function(array) {
				var inner = '';
