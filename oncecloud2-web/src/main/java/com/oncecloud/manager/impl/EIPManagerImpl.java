	...
	public JSONObject eipApply(String eipName, int uid, int eipBandwidth,
		...
		try {
			EIP eip = this.getEipDAO().applyEip(eipName, uid, eipBandwidth,
					startTime, eipUuid);
			this.getQuotaDAO().updateQuota(uid,
					"quotaIP", 1, true);
			this.getQuotaDAO().updateQuota(uid,
...
//							break;
	...
	public void eipDelete(String eipIp, int uid) {
		...
		try {
			...
			int bandwith = this.getEipDAO().abandonEip(eipIp, uid);
			this.getQuotaDAO().updateQuota(uid,
					"quotaIP", 1, false);
			if (bandwith != -1) {
				this.getQuotaDAO().updateQuota(uid,
	...
	private JSONObject bindElasticIp(int userId, String uuid, String eipIp,
			String type) {
		...
		try {
			...
			logger.info("Bind EIP: UUID [" + uuid + "] IP [" + ip + "] + EIP ["
					+ eipIp + "] Ethernet [" + eif + "]");
			boolean activeResult = activeFirewall(c,
	...
	private boolean activeFirewall(Connection c, int userId, String ip,
			String firewallId) {
		boolean result = false;
		try {
	...
	private boolean changeBandwidth(int userId, int size, String ip) {
		boolean result = false;
		try {
			Connection c = this.getConstant().getConnection(userId);
			if (ip != null) {
