...
/*
	...
	*//**
	 ...
	 * @return
		 ...
		 * public boolean bindPool(String poolUuid, String dcUuid) { boolean
		 * result = false; Session session = null; try { session =
		 * this.getSessionHelper().getMainSession(); session.beginTransaction();
		 * String queryString =
		 * "update OCPool set dcUuid = :dcUuid where poolUuid = :poolUuid";
		 * Query query = session.createQuery(queryString);
		 * query.setString("dcUuid", dcUuid); query.setString("poolUuid",
		 * poolUuid); query.executeUpdate(); session.getTransaction().commit();
		 * result = true; } catch (Exception e) { e.printStackTrace(); if
		 * (session != null) { session.getTransaction().rollback(); } } return
		 * result; }
		 *//**
	 * 从数据中心中删除资源池
	 * 
	 ...
	 * public boolean unbindPool(String poolId) { boolean result = false; OCPool
	 * pool = this.getPool(poolId); if (pool != null) { Session session = null;
	 * try { pool.setDcUuid(null); session =
	 * this.getSessionHelper().getMainSession(); session.beginTransaction();
	 * session.update(pool); session.getTransaction().commit(); result = true; }
	 * catch (Exception e) { e.printStackTrace(); if (session != null) {
	 * session.getTransaction().rollback(); } } } return result; }
