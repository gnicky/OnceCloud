	...
	$.ajax({
		...
		success: function(obj) {
			...
			$.each(obj, function(index, json) {
                ...
                if(index == 0) {
					thistr += '<td></td><td name="device">' + json.device + '</td><td name="mac">'
                    + json.mac + '</td><td name="tag">' + json.tag + '</td><td name="physical">' + json.physical + '</td></tr>';
				} else {
					thistr += '<td class="rcheck"><input type="checkbox" name="vifrow"></td><td name="device">' + json.device + '</td><td name="mac">'
                    + json.mac + '</td><td name="tag">' + json.tag + '</td><td name="physical">' + json.physical + '</td></tr>';
				}
                tableStr += thistr;
			});
...
$("#delete").on("click", function(event){
	...
	$.ajax({
		...
		data: {uuid:uuid, type:type, vifUuid:vifUuid},
		dataType: "json",
		success: function(obj){
			if(obj){
				getmaclist();
